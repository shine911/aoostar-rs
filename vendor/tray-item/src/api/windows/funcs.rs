...
/// Applies `MNS_NOTIFYBYPOS` to a popup menu so item selection is delivered
/// as `WM_MENUCOMMAND`. The root menu gets this (plus `MIM_APPLYTOSUBMENUS`)
/// at creation, but submenus created afterwards need it applied individually.
pub(crate) unsafe fn apply_notify_by_pos(hmenu: HMENU) -> bool {
    let mut info = mem::zeroed::<MENUINFO>();
    info.cbSize = mem::size_of::<MENUINFO>() as u32;
    info.fMask = MIM_STYLE;
    info.dwStyle = MNS_NOTIFYBYPOS;
    SetMenuInfo(hmenu, &info) != 0
}
...
pub(crate) unsafe extern "system" fn window_proc(
    ...
    if msg == WM_MENUCOMMAND {
        // wParam = index of the selected item, lParam = handle of the menu
        // that contains it (the root popup for top-level items, the submenu
        // popup for submenu items). Resolving via lParam is what makes
        // submenu clicks deliver their callback.
        let menu = l_param as HMENU;
        WININFO_STASH.with(|stash| {
            let stash = stash.borrow();
            let stash = stash.as_ref();
            if let Some(stash) = stash {
                let menu_id = GetMenuItemID(menu, w_param as i32) as i32;
                if menu_id != -1 {
